...
// Call Anthropic API directly (no SDK dependency)
const callAnthropic = async (prompt) => {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      system: `You are an academic performance advisor for a university examination system.
...
exports.generateFeedback = async (req, res) => {
  try {
    ...
    const qaSummary = sub.answers.map((a, i) => {
      const q = a.question;
      if (!q) return "";
...
Generate personalised academic feedback for this student.`.trim();
    ...
    const feedback = await callAnthropic(prompt);
    ...
    console.error("generateFeedback:", err);
    res.status(500).json({ message: err.message });
  }
};

// Bulk generate feedback for all published submissions of an exam
exports.bulkGenerateFeedback = async (req, res) => {
  try {
    const { examId } = req.params;

    const subs = await Submission.find({ exam: examId, published: true, aiFeedback: "" })
      .populate("exam",    "title subject totalMarks")
      .populate("student", "name rollNo")
      .populate({ path: "answers.question", model: "Question" });
    ...
    for (const sub of subs) {
      try {
        ...
        const prompt = `Student: ${sub.student?.name || "Unknown"}, Exam: "${sub.exam?.title}", Subject: ${sub.exam?.subject}, Score: ${sub.totalMarks}/${examTotalMarks} (${percentage}%), Details: ${qaSummary}. Generate personalised academic feedback.`;
        ...
        console.error(`Feedback failed for ${sub._id}:`, innerErr.message);
      }
    }
...
// Get AI feedback for a submission (student view after publish)
